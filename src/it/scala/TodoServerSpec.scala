...
class TodoServerSpec extends AnyWordSpec with Matchers with BeforeAndAfterAll with Eventually {
  ...
  override def beforeAll(): Unit = {
    ...
    eventually {
      client
        .use(_.statusFromUri(Uri.unsafeFromString(s"$urlStart/todos")))
  ...
  "Todo server" should {
    "create a todo" in {
      ...
      val request =
        Request[IO](method = Method.POST, uri = Uri.unsafeFromString(s"$urlStart/todos"))
    ...
    "update a todo" in {
      ...
      val request =
        Request[IO](method = Method.PUT, uri = Uri.unsafeFromString(s"$urlStart/todos/$id"))
    ...
    "return a single todo" in {
      ...
      client
        .use(_.expect[Json](Uri.unsafeFromString(s"$urlStart/todos/$id")))
    ...
    "delete a todo" in {
      ...
      val id          = createTodo(description, importance)
      val deleteRequest =
        Request[IO](method = Method.DELETE, uri = Uri.unsafeFromString(s"$urlStart/todos/$id"))
      client.use(_.status(deleteRequest)).unsafeRunSync() shouldBe Status.NoContent

      val getRequest =
        Request[IO](method = Method.GET, uri = Uri.unsafeFromString(s"$urlStart/todos/$id"))
    ...
    "return all todos" in {
      ...
      val json =
        client.use(_.expect[Json](Uri.unsafeFromString(s"$urlStart/todos"))).unsafeRunSync()
      ...
      // Retrieve todos
      client
        .use(_.expect[Json](Uri.unsafeFromString(s"$urlStart/todos")))
        .unsafeRunSync shouldBe json"""
        [
          {
            "id": $id1,
  ...
  private def createTodo(description: String, importance: String): Long = {
    ...
    val request = Request[IO](method = Method.POST, uri = Uri.unsafeFromString(s"$urlStart/todos"))
      .withEntity(createJson)
    val json = client.use(_.expect[Json](request)).unsafeRunSync()
    root.id.long.getOption(json).nonEmpty shouldBe true
